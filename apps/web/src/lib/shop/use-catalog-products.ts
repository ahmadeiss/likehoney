...
type Result = { key: string; products: PublicProductListItem[]; total: number; failed: boolean }
    ...
    shopClient
      ...
      .then(({ data, meta }) => {
        if (!controller.signal.aborted)
          setResult({ key, products: data, total: meta.total, failed: false })
      })
      .catch(() => {
        if (!controller.signal.aborted) setResult({ key, products: [], total: 0, failed: true })
      })
    return () => controller.abort()
  }, [page, category, search, key])
  return {
